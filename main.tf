...
resource "azurerm_linux_function_app" "func" {
  ...
  dynamic "storage_account" {
    for_each = lookup(each.value, "storage_accounts", {})

    content {
      name        = lookup(storage_account.value, "name", storage_account.key)
      type        = storage_account.value.type
      share_name  = storage_account.value.share_name
      access_key  = storage_account.value.access_key
      account_name = storage_account.value.account_name
  ...
  site_config {
    ...
    dynamic "scm_ip_restriction" {
      for_each = lookup(each.value, "scm_ip_restrictions", {})
  ...
  dynamic "sticky_settings" {
    for_each = var.instance.sticky_settings != null ? [1] : []
  ...
  dynamic "backup" {
    for_each = var.instance.backup != null ? [1] : []
  ...
  dynamic "connection_string" {
    for_each = var.instance.connection_string != null ? var.instance.connection_string : {}
...
resource "azurerm_linux_function_app_slot" "slot" {
  ...
  dynamic "auth_settings_v2" {
    ...
    content {
      ...
      # Rest of auth settings blocks...
      # (I've truncated these for brevity - you would include all of them in the actual code)
    }
  }
  ...
  dynamic "connection_string" {
    ...
    content {
      name  = lookup(connection_string.value, "name", connection_string.key)
      value = connection_string.value
      type  = connection_string.type
    }
  }
  ...
  site_config {
    ...
    dynamic "ip_restriction" {
      for_each = try(var.instance.site_config.ip_restrictions, {})
...
resource "azurerm_windows_function_app" "func" {
  ...
  dynamic "auth_settings_v2" {
    ...
    content {
      ...
      # Auth provider blocks omitted for brevity - similar structure to Linux function app
    }
  }
  ...
  site_config {
    ...
    dynamic "application_stack" {
      for_each = var.instance.site_config.application_stack != null ? [1] : []
    ...
    dynamic "cors" {
      for_each = var.instance.site_config.cors != null ? [1] : []
    ...
    dynamic "app_service_logs" {
      for_each = var.instance.site_config.app_service_logs != null ? [1] : []
...
resource "azurerm_windows_function_app_slot" "slot" {
  ...
  # Auth settings, storage accounts, connection strings, and backup configurations
  # follow the same pattern as the Linux function app slot
  # Omitted for brevity
  ...
  site_config {
    ...
    # IP restrictions and application stack blocks
    # follow the same pattern as the Linux function app slot
    # Omitted for brevity
  }
